...
Static site (Milestone 2: 7 category pages — omega-3, creatine, vitamin D3,
vitamin C, calcium, protein, plus the landing page), same non-Docker pattern
as the root splash page below — served directly by nginx from
`/var/www/realdosing`, not proxied to a container. Source lives in the
`supplement-price-app` repo (`web/` — per-category `.html`, `common.js`,
per-category `*.app.js` / `*.data.js`, `style.css`); this repo only has the
nginx vhosts, matching the FAIS source-lives-elsewhere pattern above. Deploy
with:
...
Each category's `*.data.js` is generated by the corresponding build script in
that repo from the priced CSVs — re-run it there and re-rsync after any
pricing update.

Canonical domain is now **dosinghub.com** (purchased 2026-07-20, DNS on
Cloudflare, cert via Certbot covering `dosinghub.com` + `www.dosinghub.com`).
The original `realdosing.alphatronex.com` vhost now just 301-redirects to
`dosinghub.com` — kept alive (with its own valid cert) so old links/bookmarks
still resolve. See
[nginx/dosinghub.com.conf](./nginx/dosinghub.com.conf) and
...
| realdosing (static) | nginx (no container) | — | https://dosinghub.com (realdosing.alphatronex.com redirects here) |
